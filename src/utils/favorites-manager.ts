...
export class FavoritesManager {
  ...
  static async addFavorite(
    favorite: Omit<WorkflowFavorite, 'id' | 'addedAt'>
  ): Promise<WorkflowFavorite> {
    // Validate input
  ...
  static async updateFavorite(
    id: string,
    updates: Partial<Omit<WorkflowFavorite, 'id' | 'addedAt'>>
  ): Promise<WorkflowFavorite> {
    const favorites = await this.getFavorites();
   ...
   */
  static async getFavorites(repository?: {
    owner: string;
    name: string;
  }): Promise<WorkflowFavorite[]> {
    ...
    if (repository) {
      return favorites.filter(
        (f) => f.repository.owner === repository.owner && f.repository.name === repository.name
      );
    }
  ...
  static validateFavorite(favorite: WorkflowFavorite): string[] {
    ...
    if (
      !favorite.id ||
      !/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(favorite.id)
    ) {
